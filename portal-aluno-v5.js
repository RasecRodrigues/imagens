...
function finalizarMensalidadeHomeSemDadosPortalAluno_(mensagem){
  ...
  if(competencia)competencia.textContent='Mensalidade';
  if(valor)valor.textContent='A conferir';
  if(status){
    status.textContent='Em conferência';
    status.dataset.status='EM_CONFERENCIA';
  }
  if(venc){
    venc.textContent=mensagem||'Consulte os detalhes em Pagamentos.';
  }
}
...
function carregarPartesPortalAluno() {
  ...
  /*
   * V5: a Home não chama mais obterDadosPortalAlunoComComprovantes
   * nem recalcula o financeiro ao abrir.
   * Busca somente 20 células de PortalAlunoResumo.
   */
  google.script.run
    .withSuccessHandler(d => {
      if (
        !sessao ||
        tokenAluno !== sessao ||
        sequencia !== sequenciaCargaPortalAluno
      ) return;

      const anterior = dadosPortalAlunoTela || {};
      const recebido = d || {};

      dadosPortalAlunoTela = {
        ...anterior,
        ...recebido,
        perfil:
          recebido.perfil ||
          anterior.perfil ||
          {nome:'Aluno',idade:'',foto:''},
        matriculas:
          Array.isArray(recebido.matriculas)
            ? recebido.matriculas
            : (Array.isArray(anterior.matriculas) ? anterior.matriculas : []),
        frequencia:
          recebido.frequencia ||
          anterior.frequencia ||
          {total:0,presentes:0,faltas:0,percentual:0,porTurma:[]},
        notas:
          Array.isArray(recebido.notas)
            ? recebido.notas
            : (Array.isArray(anterior.notas) ? anterior.notas : []),
        financeiro:
          Object.prototype.hasOwnProperty.call(recebido,'financeiro')
            ? recebido.financeiro
            : (anterior.financeiro || null),
        financeiroCarregado: true
      };

      renderPortalAluno(dadosPortalAlunoTela);
      mostrarAtualizacaoPortalAluno(dadosPortalAlunoTela);
    })
    .withFailureHandler(e => {
      ...
      finalizarMensalidadeHomeSemDadosPortalAluno_(
        'Valor em conferência. Abra Pagamentos para atualizar.'
      );

      ['freqPercentual','freqPresentes','freqFaltas','freqTotal']
        .forEach(id => {
    ...
    .obterHomePortalAlunoRapidoV5(sessao);
}

function carregarMensalidadeHomePortalAluno_(sessao, sequencia) {
  let respondeu = false;
  ...
  const timeout = setTimeout(() => {
    ...
    finalizarMensalidadeHomeSemDadosPortalAluno_(
      'A mensalidade está demorando para atualizar. Consulte Pagamentos ou fale com a Secretaria.'
    );
  }, 12000);
  ...
  google.script.run
    .withSuccessHandler(r => {
      ...
      dadosPortalAlunoTela.financeiro =
        r && r.financeiro
          ? r.financeiro
          : null;

      dadosPortalAlunoTela.financeiroCarregado = true;
      dadosPortalAlunoTela.dataHojePortal =
        r?.dataHojePortal || '';
      ...
      renderFinanceiroPortalAluno(
        dadosPortalAlunoTela.financeiro,
        dadosPortalAlunoTela.resumoDesatualizado
      );
    })
    .withFailureHandler(e => {
      respondeu = true;
      ...
      finalizarMensalidadeHomeSemDadosPortalAluno_(
        'Não foi possível atualizar a mensalidade agora. Consulte Pagamentos ou fale com a Secretaria.'
      );

      console.error(e);
    })
    .obterMensalidadeHomePortalAluno(sessao);
}

let financeiroDetalhadoPortalAlunoCarregando_ = false;

function carregarFinanceiroDetalhadoPortalAluno_() {
  if (
    !tokenAluno ||
    financeiroDetalhadoPortalAlunoCarregando_
  ) return;

  const sessao = tokenAluno;
  financeiroDetalhadoPortalAlunoCarregando_ = true;

  const msg =
    document.getElementById(
      'mensagemBoletoPortalAluno'
    );
  ...
  google.script.run
    .withSuccessHandler(d => {
      financeiroDetalhadoPortalAlunoCarregando_ = false;
      if (
        tokenAluno !== sessao ||
        !d
      ) return;
...
renderPortalAluno = function(dados){
  ...
  setTimeout(carregarAvaliacaoPortalAlunoTela_, 350);
};
...
function mostrarTelaPortalAluno(nome){
  ...
  if(nome==='pagamentos'){
    setTimeout(carregarFinanceiroDetalhadoPortalAluno_,60);
  }
  if(nome==='pedidos')setTimeout(()=>carregarPedidosPortalAluno(false),60);
...
function renderResumoHomePortalAluno(d){
  d=d||{};
  const p=d.perfil||{},f=d.frequencia||{},fin=d.financeiro||null;
  const dadosFinanceirosJaCarregados=
    Boolean(d.dataHojePortal||d.atualizadoEm||d.financeiroCarregado===true);
  ...
  if(valor){
    const principal=fin
      ...
      :null;
    valor.textContent=fin?moeda(principal):(dadosFinanceirosJaCarregados?'A conferir':'Carregando...');
  }
  ...
  if(status){
    status.textContent=fin?(rotulos[fin.status]||'Em conferência'):(dadosFinanceirosJaCarregados?'Em conferência':'Carregando');
    status.dataset.status=fin?.status||'';
  }
  if(venc){
    if(!fin&&dadosFinanceirosJaCarregados){
      venc.textContent='Valor em conferência. Consulte os detalhes em Pagamentos.';
    }else{
...
function enviarSolicitacaoCarteirinhaPortal(evento){
  ...
  const enviar=comprovantePayload=>{
    ...
    google.script.run.withSuccessHandler(r=>{
      ...
      const concluir=()=>{
        btn.disabled=false;
        msg.textContent='✓ Compra realizada com sucesso! Acompanhe o andamento pelo Portal do Aluno ou com a Secretaria. A carteirinha será confeccionada quando o lote atingir o mínimo de 10 pedidos.';
        fotoCarteirinhaNovaPortalV3=null;
...
/* =========================================================
   MEUS PEDIDOS — ACOMPANHAMENTO POR STATUS
   ========================================================= */
let pedidosPortalAlunoCarregados_ = false;
let pedidosPortalAlunoCarregando_ = false;

function nomeStatusPedidoPortalAluno_(status){
  const nomes={
    SOLICITADO:'Pedido recebido',
    ACEITO:'Aceito',
  ...
  return nomes[String(status||'').toUpperCase()]||String(status||'Em análise').replace(/_/g,' ');
}

function nomeProdutoPedidoPortalAluno_(item){
  const tipo=String(item?.tipoProduto||'Produto').toUpperCase();
  const nomes={
    CAMISA:'Camisa',
    CALCA:'Calça',
...
function carregarPedidosPortalAluno(forcar){
  const lista=document.getElementById('listaPedidosPortalAluno');
  if(!lista||!tokenAluno)return;

  if(pedidosPortalAlunoCarregando_)return;
  if(pedidosPortalAlunoCarregados_&&!forcar)return;

  const sessao=tokenAluno;
  pedidosPortalAlunoCarregando_=true;
  lista.innerHTML='<div class="pedido-estado-portal">Carregando seus pedidos...</div>';

  google.script.run
    .withSuccessHandler(r=>{
      pedidosPortalAlunoCarregando_=false;
      if(tokenAluno!==sessao)return;
      pedidosPortalAlunoCarregados_=true;
      renderizarPedidosPortalAluno_(r?.pedidos||[]);
    })
    .withFailureHandler(e=>{
      pedidosPortalAlunoCarregando_=false;
      if(tokenAluno!==sessao)return;
      lista.innerHTML=
        '<div class="pedido-estado-portal">'+
        escAluno(e?.message||'Não foi possível carregar seus pedidos.')+
        '</div>';
    })
    .listarPedidosPortalAluno(sessao);
}

function renderizarPedidosPortalAluno_(pedidos){
  const lista=document.getElementById('listaPedidosPortalAluno');
  if(!lista)return;

  if(!Array.isArray(pedidos)||!pedidos.length){
    lista.innerHTML=
      '<div class="pedido-estado-portal">Você ainda não possui encomendas registradas.</div>';
    return;
  }

  const etapas=[
    {status:'SOLICITADO',nome:'Pedido recebido'},
    {status:'ACEITO',nome:'Aceito'},
    {status:'EM_PRODUCAO',nome:'Em produção'},
    {status:'DISPONIVEL',nome:'Disponível'},
    {status:'ENTREGUE',nome:'Entregue'}
  ];

  lista.innerHTML=pedidos.map(pedido=>{
    const itens=(pedido.itens||[]).map(item=>{
      const status=String(item.status||'SOLICITADO').toUpperCase();
      const etapa=Number(item.etapa||1);
      const produto=nomeProdutoPedidoPortalAluno_(item);
      const detalhes=[
        item.modelo,
        item.tamanho?'Tamanho '+item.tamanho:'',
        item.remessa?'Remessa '+item.remessa:''
      ].filter(Boolean).join(' · ');

      const timeline=status==='CANCELADO'
        ? '<div class="pedido-cancelado-portal">Pedido cancelado</div>'
        : '<div class="pedido-timeline-portal">'+
          etapas.map((e,i)=>{
            const numero=i+1;
            const classe=numero<etapa?'concluida':numero===etapa?'atual':'';
            return '<div class="pedido-etapa-portal '+classe+'">'+escAluno(e.nome)+'</div>';
          }).join('')+
          '</div>';

      return '<article class="pedido-item-portal">'+
        '<div class="pedido-item-cabecalho-portal">'+
          '<div>'+
            '<h4>'+escAluno(produto)+'</h4>'+
            '<p>'+escAluno(detalhes||'Acompanhamento da encomenda')+'</p>'+
            (item.remessa?'<span class="pedido-remessa-portal">Remessa '+escAluno(item.remessa)+'</span>':'')+
          '</div>'+
          '<span class="pedido-status-chip-portal">'+escAluno(nomeStatusPedidoPortalAluno_(status))+'</span>'+
        '</div>'+
        timeline+
      '</article>';
    }).join('');

    return '<section class="pedido-grupo-portal">'+
      '<div class="pedido-grupo-topo-portal">'+
        '<div><strong>Pedido '+escAluno(pedido.idPedido||'')+'</strong>'+
        '<small>'+(pedido.data?escAluno(pedido.data):'Data não informada')+'</small></div>'+
      '</div>'+
      itens+
    '</section>';
  }).join('');
...
function mostrarConfirmacaoPedidoLojaPortal(texto){
 ...
 const itens=document.getElementById('lojaCarrinhoItens');
 if(!el)return;

 const temTexto=Boolean(String(texto||'').trim());
 el.textContent=temTexto?String(texto):'';
 el.classList.toggle('oculto',!temTexto);

 // Depois de uma compra concluída, não mostra "Seu carrinho está vazio".
 if(itens)itens.style.display=temTexto?'none':'';

 if(temTexto)el.scrollIntoView({behavior:'smooth',block:'nearest'});
}
 ...
 const enviar=comprovante=>{
  try{
   google.script.run.withSuccessHandler(r=>{
    ...
    pedidosPortalAlunoCarregados_=false;
    carrinhoLojaPortal=[];
    ...
    mostrarConfirmacaoPedidoLojaPortal(
      ...
      'Acompanhe o andamento do seu pedido pelo Portal do Aluno ou com a Secretaria. ' +
